export const initialPomodoroState = {
    ...
    breakTime: 5,
    eventId: '',
    blocks: calcularBlocosFoca(30).blocos,
    breakCount: calcularBlocosFoca(30).qtdIntervalos,
};
 ...
 */
function calcularBlocosFoca(tempoTotal) {
    ...
    return {
        ...
        blocos: blocos, // Array com o tempo de cada bloco (ex: [28, 27])
        qtdIntervalos: qtdIntervalos,
    };
}
...
export function pomodoroReducer(state, action) {
    switch (action.type) {
        case 'INCREMENT': {
            let newFocusTime = state.focusTime;
            if (newFocusTime >= 50) {
                newFocusTime += 15;
            } else {
                newFocusTime += 5;
            }
            const { blocos: blocks, qtdIntervalos: breakCount } = calcularBlocosFoca(newFocusTime);
            return { ...state, focusTime: newFocusTime, blocks, breakCount };
        }
