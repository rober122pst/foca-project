...
export async function register(req, res) {
    ...
    if (name.length < 3 || name.length > 50) {
        return res.status(400).json({ message: 'Nickname deve ter entre 3 e 20 caracteres' });
    }
    if (!/\S+@\S+\.\S+/.test(email)) {
    ...
    if (!acceptTerms) {
        return res.status(400).json({ message: 'Você deve aceitar os termos de serviço' });
    }
...
export async function login(req, res) {
    ...
    try {
        ...
        if (!isPasswordValid) { return res.status(404).json({ message: 'Email ou senha inválidos' }) }
